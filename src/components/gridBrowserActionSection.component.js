...
export default function GridBrowserActionSection(props) {
    ...
    return(
        <>
            <Grid key={props.value} item xs={ isBrowser ? 4 : 3} style={ isBrowser ? { display: 'block'} : { display : 'none'}}>
                <Grid container justify="left" spacing={0}>
                    <Grid key={props.value} item xs={3}>
                    ...
                    <Grid key={props.value} item xs={4}>
                        <Paper className={classes.body} onClick= {() => setCalendar(!isCalendarRequired)} >
